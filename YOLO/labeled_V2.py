...
def click_mouse(event, x, y, flags, param):
    ...
    if not bandClick:
        if bandRect:
            ...
            imgLine = img.copy()
        cv2.line(imgLine, (0, y), (w, y), (128, 128, 128), 2)
        cv2.line(imgLine, (x, 0), (x, h), (128, 128, 128), 2)
        cv2.imshow('image',imgLine)
    if event == cv2.EVENT_LBUTTONDOWN and not bandClass:
        xmin = x
        ymin = y
        bandClick = True
    if bandClick:
        imgRect = img.copy()
        cv2.rectangle(imgRect, (xmin, ymin), (x, y), (255, 0, 0), 2)    
        cv2.imshow('image',imgRect)
        bandRect = True
    if event == cv2.EVENT_LBUTTONUP and not bandClass:
        bandClick = False
        bandClass = True
        ...
        if labelOK and (label == 'a' or label == 'b' or label == 'c' or label == 'd'):
            labelOK = False
            tagged = True
            xmax = x
            ymax = y
            ...
            cv2.putText(img, label, (xmin, ymin-1), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            cv2.imshow('image',img)
        bandClass = False
        bandRect = False
    ...
    with open(fileTxt) as File:
        ...
        for row in reader:
            ...
            if startY < 0:
                startY = 0
            imgRect = img.copy()
            cv2.rectangle(imgRect, (startX, startY), (endX, endY), (255, 0, 0), 2)
            cv2.imshow('image',imgRect)
            bandRect = True
            bandClass = True
            ...
            if labelOK and (label == 'a' or label == 'b' or label == 'c' or label == 'd'):
                ...
                cv2.putText(img, label, (startX, startY-1), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                cv2.imshow('image',img)
...
def folder(path, predictOK = False):
    ...
    for p in glob.glob(path + "\*.jpg"):
        ...
        print (p, "  countFrames:  ",countFrames)
        i = i + 1
        if i == jumps+1 or i == 0:
            i = 0
            image = cv2.imread(p)
            ...
            while cap.isOpened():        
                ...
                if i == jumps+1 or i == 0:
                    ...
                    if key == ord('q'):
                        ...
                        break
                    if tagged:                       
                        imgR = cv2.resize(image, (0,0), fx=PERCENT, fy=PERCENT)
                        cv2.imwrite(p2, imgR)
                        a = a + 1
                f = open("countFrames.txt", 'w+')
                f.write(str(j) + '\n')
